...
// ─── Config ──────────────────────────────────────────
const TOTAL_SECTIONS = 8; // seções reais (sem a tela de conclusão)
const WA_NUMBER = '5521987151641';
...
// ─── Partículas (reutiliza lógica da landing page) ───
(function initParticles() {
  ...
  function mk() {
    return { x: rand(0,W), y: rand(0,H), vx: rand(-0.15,0.15), vy: rand(-0.15,0.15),
             r: rand(0.7,1.8), alpha: rand(0.1,0.4), aDir: Math.random()>.5?1:-1, aSpd: rand(.002,.005) };
  }
...
// ─── Coleta respostas da seção atual ─────────────────
function collectSection(idx) {
  ...
  sec.querySelectorAll('input, textarea').forEach(el => {
    if (el.name) answers[el.name] = el.value.trim();
  });
...
function goTo(next, direction = 'forward') {
  ...
  leaving.addEventListener('animationend', () => {
    // 2. Esconde completamente a seção que saiu
    leaving.classList.remove('active', leaveClass);
    leaving.style.display = '';

    // 3. Reseta o scroll do container ANTES de mostrar a nova seção
    quizWrapper.scrollTo({ top: 0, behavior: 'instant' });

    // 4. Exibe e anima entrada da nova seção
...
btnNext.addEventListener('click', () => {
  if (current < TOTAL_SECTIONS - 1) {
    // navega para próxima seção normalmente
    goTo(current + 1, 'forward');
  } else if (current === TOTAL_SECTIONS - 1) {
    // última seção → coleta, vai para tela de conclusão e dispara webhook
    collectSection(current);
    goTo(TOTAL_SECTIONS, 'forward');
    sendToWebhook();
  }
});
...
// ─── Tecla Enter / setas de teclado ──────────────────
document.addEventListener('keydown', (e) => {
  if (e.target.tagName === 'TEXTAREA') return;
  if ((e.key === 'Enter' || e.key === 'ArrowRight') && current < TOTAL_SECTIONS) {
    btnNext.click();
  }
...
function showDoneState(id) {
  ['state-sending', 'state-success', 'state-error'].forEach(s => {
    ...
    if (el) el.style.display = s === id ? (s === 'state-sending' ? 'flex' : 'flex') : 'none';
  });
}
...
async function sendToWebhook() {
  ...
  const payload = {
    ...
    origem: 'andersonquintanilha.com.br',
    ...answers,
  };
